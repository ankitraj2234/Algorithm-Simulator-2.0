...
public class AnalysisController {
    ...
    @FXML private ComboBox<String> algoAComboBox;
    @FXML private ComboBox<String> algoBComboBox;
    @FXML private Button analyzeButton;
    @FXML private Button swapButton;

    // Summary labels
    @FXML private Label nameALabel, bestALabel, avgALabel, worstALabel, spaceALabel;
    @FXML private Label nameBLabel, bestBLabel, avgBLabel, worstBLabel, spaceBLabel;

    // Charts
    @FXML private LineChart<Number, Number> complexityChart;
    @FXML private NumberAxis xAxis, yAxis;
    @FXML private BarChart<String, Number> performanceChart;
    @FXML private CategoryAxis perfXAxis;
    @FXML private NumberAxis perfYAxis;

    // Table and narrative
    @FXML private TableView<Row> comparisonTable;
    @FXML private TableColumn<Row, String> nameColumn;
    @FXML private TableColumn<Row, String> bestColumn;
    @FXML private TableColumn<Row, String> avgColumn;
    @FXML private TableColumn<Row, String> worstColumn;
    @FXML private TableColumn<Row, String> spaceColumn;
    @FXML private TextArea analysisText;

    // Domain table (always visible)
    @FXML private TableView<Row> domainTable;
    @FXML private TableColumn<Row, String> dNameColumn;
    @FXML private TableColumn<Row, String> dBestColumn;
    @FXML private TableColumn<Row, String> dAvgColumn;
    @FXML private TableColumn<Row, String> dWorstColumn;
    @FXML private TableColumn<Row, String> dSpaceColumn;
    ...
    public void initialize() {
        ...
        if (!repo.getCategories().isEmpty()) {
            ...
            populateAlgorithms();
            if (!algoAComboBox.getItems().isEmpty()) algoAComboBox.getSelectionModel().select(0);
            if (algoBComboBox.getItems().size() > 1) algoBComboBox.getSelectionModel().select(1);
            analyze();
        }
    }
    ...
    private void setupDomainTable() {
        if (dNameColumn != null) dNameColumn.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().name()));
        if (dBestColumn != null) dBestColumn.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().best()));
        if (dAvgColumn != null) dAvgColumn.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().avg()));
        if (dWorstColumn != null) dWorstColumn.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().worst()));
        if (dSpaceColumn != null) dSpaceColumn.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().space()));
    }
    ...
    private void fillSummary(AlgorithmRecord r, Label name, Label best, Label avg, Label worst, Label space) {
        if (name != null) name.setText(r.getName());
        if (best != null) best.setText(r.getBest());
        if (avg != null) avg.setText(r.getAverage());
        if (worst != null) worst.setText(r.getWorst());
        if (space != null) space.setText(r.getSpace());
    }
    ...
    private String buildNarrative(String cat, AlgorithmRecord A, AlgorithmRecord B) {
        ...
        if (cmp < 0) {
            sb.append("✅ Prefer ").append(A.getName()).append(" for typical inputs based on average-case complexity.\n");
            sb.append("   ").append(A.getName()).append(" has better average performance: ").append(A.getAverage()).append(" vs ").append(B.getAverage()).append("\n\n");
        } else if (cmp > 0) {
            sb.append("✅ Prefer ").append(B.getName()).append(" for typical inputs based on average-case complexity.\n");
            sb.append("   ").append(B.getName()).append(" has better average performance: ").append(B.getAverage()).append(" vs ").append(A.getAverage()).append("\n\n");
        } else {
    ...
    private double evalComplexity(String c, int n, String cat) {
        ...
        switch (k) {
            case "O(1)" -> { return 1; }
            case "O(log n)" -> { return Math.log(Math.max(2, n)) / Math.log(2); }
            case "O(log log n)" -> { return Math.log(Math.max(2, Math.log(Math.max(2, n)) / Math.log(2))) / Math.log(2); }
            case "O(n)" -> { return n; }
            case "O(n log n)" -> { return n * (Math.log(Math.max(2, n)) / Math.log(2)); }
            case "O(n^2)", "O(n²)" -> { return n * n; }
            case "O(n^3)", "O(n³)" -> { return n * n * n; }
            case "O(2^n)" -> { return Math.pow(2, Math.min(n, 20)); }
            // Graph forms: approximate with n or n^2 to visualize
            case "O(v+e)" -> { return n; }
            case "O(v^2)" -> { return n * n; }
            default -> { return n; }
        }
    }
    ...
    private String normalize(String raw) {
        ...
        if (s.contains("loglog")) return "O(log log n)";
        if (s.contains("log") && s.startsWith("o(nlog")) return "O(n log n)";
        if (s.contains("2^n")) return "O(2^n)";
        if (s.contains("n^2") || s.contains("n²")) return "O(n^2)";
        if (s.contains("n^3") || s.contains("n³")) return "O(n^3)";
        if (s.contains("(v+e)")) return "O(V+E)".toLowerCase(Locale.ROOT);
        if (s.contains("(v^2)") || s.contains("(v²)")) return "O(V^2)".toLowerCase(Locale.ROOT);
