...
public class StackController {
    ...
    @FXML
    private Button pushButton, popButton, peekButton, clearButton;
    @FXML
    private Label statusLabel, sizeLabel, topLabel, capacityLabel;
    @FXML
    VBox stackContainer;
    @FXML
    ...
    private void pushElement() {
        ...
        if (input.isEmpty()) {
            AlertHelper.showWarning("Invalid Input", "Please enter a number to push onto the stack.");
            return;
        }
        ...
        try {
            ...
            if (stackModel.isFull()) {
                AlertHelper.showWarning("Stack Full",
                        "Cannot push - Stack has reached maximum capacity of " + stackModel.getCapacity());
                return;
            }
    ...
    private void popElement() {
        ...
        if (stackModel.isEmpty()) {
            AlertHelper.showWarning("Stack Empty", "Cannot pop from an empty stack.");
            return;
        }
        ...
        // Show popped value
        AlertHelper.showInfo("Element Popped", "Popped value: " + poppedValue);
    ...
    private void peekElement() {
        if (stackModel.isEmpty()) {
            AlertHelper.showWarning("Stack Empty", "Cannot peek into an empty stack.");
            return;
        }
        ...
        AlertHelper.showInfo("Peek Result", "Top element: " + topValue);
    ...
    private void clearStack() {
        ...
        if (stackModel.isEmpty()) {
            AlertHelper.showWarning("Stack Empty", "Stack is already empty.");
            return;
        }
        ...
        confirmAlert.setContentText(
                "Are you sure you want to clear all " + stackModel.size() + " elements from the stack?");
    ...
    private void updateDisplay() {
        if (statusLabel != null) {
            statusLabel
                    .setText(stackModel.isEmpty() ? "Stack is Empty" : "Stack has " + stackModel.size() + " elements");
        }
