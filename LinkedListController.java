...
import javafx.stage.Modality;   // ← ADD THIS
...
public class LinkedListController {
    ...
    @FXML private Button insertBeginButton, insertEndButton, insertPositionButton;
    @FXML private Button deleteValueButton, deletePositionButton, searchButton;
    @FXML private Button clearButton;
    @FXML private HBox listVisualization;
    ...
    private void searchValue() {
        ...
        try {
            ...
            animateSearch(value, (position) -> {
                ...
                logOperation("SEARCH " + value + " | Result: " + (position != -1 ? "Found at " + position : "Not found"));
            });
    ...
    private void updateDisplay() {
        Platform.runLater(() -> {
            if (statusLabel != null) {
                statusLabel.setText(listModel.isEmpty() ?
                        "LinkedList is Empty" :
    ...
    private void updateControlStates() {
        ...
        if (insertBeginButton != null) insertBeginButton.setDisable(isRunning);
        if (insertEndButton != null) insertEndButton.setDisable(isRunning);
        if (insertPositionButton != null) insertPositionButton.setDisable(isRunning);
        if (deleteValueButton != null) deleteValueButton.setDisable(isEmpty || isRunning);
        if (deletePositionButton != null) deletePositionButton.setDisable(isEmpty || isRunning);
        ...
        if (inputField != null) inputField.setDisable(isRunning);
        if (positionField != null) positionField.setDisable(isRunning);
        if (searchField != null) searchField.setDisable(isRunning);
    }
    ...
    private static class NodeElement {
        ...
        public Label getArrow() { return arrow; }
